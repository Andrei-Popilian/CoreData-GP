...
private extension CoreDataManager {
  ...
  func performAsync(_ block: @escaping (NSManagedObjectContext) -> Void, on context: NSManagedObjectContext) {
    context.perform {
      block(context)
    }
  }
  
  func performSync(_ block: (NSManagedObjectContext) -> Void, on context: NSManagedObjectContext) {
  ...
  func loadPersistentStore(completion: @escaping () -> Void) {
    ...
    migrateStoreIfNeeded(completion: {
  ...
  func createPersistentContainer(modelName: String, storeType: String) -> PersistentContainer {
