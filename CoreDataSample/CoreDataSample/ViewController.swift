...
final class ViewController: UIViewController {
  ...
  @IBAction func addMore(_ sender: Any) {
    ...
    coreDataStack.performOnBackgroundAsync { [weak self] (context) in
  ...
  @IBAction func decodeAndAdd(_ sender: Any) {
    
    coreDataStack.performOnBackgroundAsync { (context) in
      
      let json = ["firstName": "Codable Test"]
      let data = try? JSONSerialization.data(withJSONObject: json, options: .prettyPrinted)
      guard let jsonData = data else { return }
      
      _ = Event.parsedToManagedObjectFromJson(jsonData, onContext: context, decoder: JSONDecoder())
        .mapError { (failure) -> Error in
          //do something it failed
          switch failure {
      ...
      .map { (theObject) in
        print("We have the object \(theObject)")
      }
    }
  }
  ...
  @IBAction func deleteItem(_ sender: Any) {
    ...
    coreDataStack.performOnBackgroundAsync { [weak self] (backgroundContext) in
      guard let self = self else { return }
